...
func main() {
	shared.Run(func() any {
		grid := grid{}
		idx := 0
		shared.Open("10.txt", func(fileScanner *bufio.Scanner) {
...
func checkLayer(value int, grid grid) int {
	...
	checkNeighbour := func(x, y int) (result []string) {
		...
		result = append(result, element.atteignableSubmit...)
		return
	}
	for idx, row := range grid {
		for jdx, element := range row {
			...
			atteignableSummits = append(atteignableSummits, checkNeighbour(idx, jdx-1)...)
			slices.Sort(atteignableSummits)
			element.atteignableSubmit = slices.Compact(atteignableSummits)
